...
export class TablesRemoteDataService {
  ...
  Getgrades(
    ...
    return this.http.get<PaginationResponseApi>(
      `${this.ApiUrlgeral}GetGrades?nomeno=${nonome}&currentNumber=${currentNumber}&pagesize=${pagesize}`
    );
  }
  ...
  GerarRelatorioParam(nomeficheiro: string): Observable<Resposta<ReportParam>> {
    return this.http.post<Resposta<ReportParam>>(
      `${this.ApiUrlgeral}Proc2/LeituraDeFicheiros`,
      nomeficheiro
    );
  }
}
